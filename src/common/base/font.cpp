 ...
 *
 * Native 16x16 HZK16 glyphs (no downsampling), stored as 8bpp index
 * data in _CjkPix and uploaded as a 1024x1024 PSMT4 (4bpp) atlas with
 * a CT32 palette.  Dots map to CLUT index 1 (alpha 0xFF) / index 0
 * (transparent).
 *
 ...
 *
 * The 8bpp source data is repacked to 4bpp at FontInit time (two
 * texels per byte, low nibble = even/left column - the PS2 PSMT4
 * nibble order) into a .bss scratch buffer, so the generated data
...
#define CJK_ATLAS_SRC_BYTES (CJK_ATLAS_W * CJK_ATLAS_H)   /* 8bpp source */
...
void FontInit(Uint32 uVramAddr)
{
    ...
    /* Repack the 8bpp index data to 4bpp (PSMT4) in the .bss scratch
       buffer: two texels per byte, low nibble = even (left) column -
       the PS2 PSMT4 nibble order.  Done once at startup. */
    {
        Uint32 i;
