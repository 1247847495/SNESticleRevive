...
{
	uint8_t* pixels = scanline_pixels;
	...
	/* AURORA_V3_SAFE_QN_BG_MEMSET_20260828:
	 * fill repeats one byte in all lanes, so this is byte-identical. */
	for ( int n = count; n--; )
	{
		memset( pixels, (int)(fill & 0xFF), image_width );
		pixels += scanline_row_bytes;
	}
}
...
void Nes_Ppu_Rendering::draw_sprites_( int begin, int end )
{
	...
	/* AURORA_V15_MULTICORE_SPRITE_LIMIT_20260824: per-screen mode keeps only the first N OAM entries. */
	while ( index < 0x100 && index < sprite_screen_limit * 4 );
}
