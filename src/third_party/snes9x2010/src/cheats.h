 ...
 * Standalone Aurora has no SNES9x cheat service. The stock structure keeps
 * 128 KiB WRAM + 64 KiB SRAM + 8 KiB IRAM mirrors resident even with zero
 * cheats. Keep one inert entry for ABI/source tolerance and only the pointers
 * required by the no-op initialiser. */
#ifdef PS2
typedef struct
{
	struct SCheat c[1];
