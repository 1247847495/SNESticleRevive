/* AURORA_SNES9X2010_V2_PS2LEAN_20260824 -- cheats are not exposed by standalone Aurora. */
#ifdef PS2
#include <stdint.h>
#include "snes9x.h"
#include "memmap.h"
...
void S9xDeleteCheats(void) { Cheat.num_cheats = 0; }
...
{ (void)code; (void)address; (void)byte; return "Cheats disabled on Aurora PS2"; }
const char *S9xProActionReplayToRaw(const char *code, uint32_t *address, uint8_t *byte)
{ (void)code; (void)address; (void)byte; return "Cheats disabled on Aurora PS2"; }
const char *S9xGoldFingerToRaw(const char *code, uint32_t *address, uint8_t *sram,
                              uint8_t *num_bytes, uint8_t bytes[3])
{
    (void)code; (void)address; (void)sram; (void)num_bytes; (void)bytes;
    return "Cheats disabled on Aurora PS2";
}
#else
/***********************************************************************************
