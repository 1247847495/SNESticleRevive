 ...
 * MSU-1 is a homebrew sidecar/data+PCM expansion, not SNES cartridge hardware
 * needed by the target library. Standalone Aurora deliberately omits it so the
 * PS2 build does not carry file_stream, ZIP/DEFLATE and PCM streaming code. */
#ifdef PS2
#include <stddef.h>
#include <stdint.h>
#include <string.h>
...
struct SMSU1 MSU1;
void S9xResetMSU1(void) { memset(&MSU1, 0, sizeof(MSU1)); Settings.MSU1 = FALSE; }
void S9xMSU1Init(void) { S9xResetMSU1(); }
void S9xMSU1DeInit(void) { S9xResetMSU1(); }
uint8_t S9xMSU1ROMExists(void) { return FALSE; }
void S9xMSU1SetROMPath(const char *rom_path) { (void)rom_path; }
uint8_t S9xMSU1ReadPort(uint8_t port) { (void)port; return 0; }
void S9xMSU1WritePort(uint8_t port, uint8_t byte) { (void)port; (void)byte; }
void S9xMSU1Mix(int16_t *buffer, size_t sample_count, uint32_t output_rate)
{ (void)buffer; (void)sample_count; (void)output_rate; }
