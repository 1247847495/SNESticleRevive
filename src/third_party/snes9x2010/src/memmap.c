 ...
 * PS2/EE memory profile.
 *
 * The libretro core normally reserves MAX_ROM_SIZE (8 MiB) for every game,
 * and the first runtime fix still had to keep a 6 MiB floor because C4,
 * OBC1 and BS-X scratch pointers were aliases at fixed +3/+4 MiB offsets.
 * Those aliases are not ROM storage. On PS2 give C4/OBC1 their real 8 KiB
 * work RAM and do not manufacture out-of-range BS-X pointers in a normal
 * SNES cartridge run. The ROM heap can then follow the actual image size.
 * A 64 KiB zero guard stays after the visible ROM for conservative legacy
 * speculative reads; it is allocation-only and is never reported as ROM.
 */
#ifdef PS2
#define AURORA_PS2_ROM_GUARD 0x10000U
static uint32_t s_AuroraRomCapacity = MAX_ROM_SIZE;
static uint8_t *s_AuroraC4RAM = NULL;
static uint8_t *s_AuroraOBC1RAM = NULL;
static uint8_t s_AuroraBsxMode = FALSE;

void S9xAuroraSetRomCapacity(uint32_t contentBytes, int bsxMode)
{
	uint32_t cap = contentBytes;

	s_AuroraBsxMode = bsxMode ? TRUE : FALSE;
	/* Header scoring reads through $FFC0/$FFxx even on tiny/homebrew images. */
	if (cap < 0x10000U) cap = 0x10000U;
	/* Correctly identified BS-X keeps the historical +3/+4 MiB layout. */
	if (s_AuroraBsxMode && cap < 0x600000U) cap = 0x600000U;
	/* FileLoader itself accepts a possible 512-byte copier header on top. */
	if (cap > MAX_ROM_SIZE) cap = MAX_ROM_SIZE;
	cap = (cap + 0xffffU) & ~0xffffU;
	if (cap > MAX_ROM_SIZE) cap = MAX_ROM_SIZE;
	s_AuroraRomCapacity = cap;
}
static uint32_t S9xAuroraRomCapacity(void)
{
	return s_AuroraRomCapacity;
}
#else
#define AURORA_PS2_ROM_GUARD 0U
static uint32_t S9xAuroraRomCapacity(void)
{
	return MAX_ROM_SIZE;
}
...
uint8_t Init (void)
{
	...
	Memory.ROM  = (uint8_t *) calloc(1, S9xAuroraRomCapacity() + 0x200 + 0x8000 + AURORA_PS2_ROM_GUARD);
#ifdef PS2
	s_AuroraC4RAM   = (uint8_t *) calloc(1, 0x2000);
	s_AuroraOBC1RAM = (uint8_t *) calloc(1, 0x2000);
#endif
...
#ifdef PS2
	printf("[S9X2010][PS2] ROM heap=%u KiB + guard=%u KiB; C4/OBC1=8+8 KiB\n",
	       (unsigned)(S9xAuroraRomCapacity() >> 10),
	       (unsigned)(AURORA_PS2_ROM_GUARD >> 10));
#endif

	if (!Memory.RAM || !Memory.SRAM || !Memory.VRAM || !Memory.ROM ||
#ifdef PS2
			!s_AuroraC4RAM || !s_AuroraOBC1RAM ||
#endif
...
#ifdef PS2
	/* AURORA_SNES9X2010_V1_PS2LEAN_20260824: real C4/OBC1 scratch, not fake +4 MiB aliases. */
	Memory.C4RAM   = s_AuroraC4RAM;
	Memory.OBC1RAM = s_AuroraOBC1RAM;
	if (s_AuroraBsxMode)
	{
		/* Preserve direct .bsx/BS-X compatibility only when requested. */
		Memory.BIOSROM = Memory.ROM + 0x300000;
		Memory.BSRAM   = Memory.ROM + 0x400000;
	}
	else
	{
		Memory.BIOSROM = NULL;
...
void Deinit (void)
{
	...
	if (s_AuroraOBC1RAM)
	{
		free(s_AuroraOBC1RAM);
		s_AuroraOBC1RAM = NULL;
	}
	Memory.C4RAM = NULL;
	Memory.OBC1RAM = NULL;
	Memory.BIOSROM = NULL;
...
uint8_t LoadROM (void)
{
	...
	memset(Memory.ROM, 0, S9xAuroraRomCapacity());
...
again:
	...
	totalFileSize = FileLoader(Memory.ROM, (int32_t)S9xAuroraRomCapacity());
	if (!totalFileSize)
	{
...
#ifdef PS2
	/* AURORA_SNES9X2010_V1_PS2LEAN_20260824: ordinary cartridges do not need the historical
	 * +3/+4 MiB BS scratch aliases. Direct .bsx / recognized BS-X BIOS
	 * still opts into that layout and runs the original parser. */
	if (s_AuroraBsxMode)
		S9xInitBSX();
	else
	{
