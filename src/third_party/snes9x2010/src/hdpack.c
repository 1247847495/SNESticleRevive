 ...
 * Aurora V1 does not expose HD texture packs. Keep the renderer ABI intact
 * for tile.c/libretro.c, but do not pull PNG/WebP/DDS/VFS machinery into the
 * PS2 ELF. Other platforms compile the original implementation unchanged. */
#ifdef PS2
#include "hdpack.h"
int S9xHdPackInit(const char *rom_path) { (void)rom_path; return 0; }
