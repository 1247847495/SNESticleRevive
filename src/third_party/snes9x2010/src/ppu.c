...
extern uint8_t S9xAuroraLayerMask(void);
extern uint8_t S9xAuroraHackFlags(void);
extern int S9xAuroraObjTileBudget(void);
extern int S9xAuroraObjScreenBudget(void);
extern uint32_t S9xAuroraFramePhase(void);

#define AURORA_HACK_COLOR_MATH_OFF 0x01U
#define AURORA_HACK_WINDOWS_OFF    0x02U
#define AURORA_COLOR_MATH(value)     ((S9xAuroraHackFlags() & AURORA_HACK_COLOR_MATH_OFF) ? 0 : (value))

static uint8_t s_AuroraObjScreenSelected[128];

static int S9xAuroraObjVisibleY(uint8_t y, int height)
{
    while (height-- > 0)
    {
...
static void S9xAuroraPrepareObjScreenLimit(int small_width, int small_height,
                                           int large_width, int large_height)
{
    uint8_t candidates[128];
    int count = 0;
    int budget = S9xAuroraObjScreenBudget();
    int step;

    memset(s_AuroraObjScreenSelected, budget >= 128 ? 1 : 0,
           sizeof(s_AuroraObjScreenSelected));
    if (budget >= 128)
        return;

    for (step = 0; step < 128; ++step)
    {
        uint8_t sprite = (uint8_t)((PPU.FirstSprite + step) & 0x7f);
        int width = PPU.OBJ[sprite].Size ? large_width : small_width;
        int height = PPU.OBJ[sprite].Size ? large_height : small_height;
        int x = PPU.OBJ[sprite].HPos;
        if (x == -256) x = 256;
        if (x < 256 && x > -width &&
            S9xAuroraObjVisibleY((uint8_t)PPU.OBJ[sprite].VPos, height))
            candidates[count++] = sprite;
    }

    if (count > 0)
    {
        int keep = count < budget ? count : budget;
        int start = count > budget
            ? (int)(((S9xAuroraFramePhase() % (uint32_t)count) *
                     (uint32_t)budget) % (uint32_t)count)
            : 0;
        for (step = 0; step < keep; ++step)
            s_AuroraObjScreenSelected[candidates[(start + step) % count]] = 1;
    }
}
...
void SetupOBJ (void)
{
	...
	S9xAuroraPrepareObjScreenLimit(
	...
	if (!PPU.OAMPriorityRotation || !(PPU.OAMFlip & PPU.OAMAddr & 1)) /* normal case*/
	{
		...
		for ( i = 0; i < SNES_HEIGHT_EXTENDED; i++)
		{
			...
			GFX.OBJLines[i].Tiles = S9xAuroraObjTileBudget();
			for ( j = 0; j < 32; j++)
		...
		do
		{
			if (!s_AuroraObjScreenSelected[S])
			{
				S = (S + 1) & 0x7f;
				continue;
			}
	...
	else /* evil FirstSprite+Y case*/
	{
      ...
      memset(AnyOBJOnLine, FALSE, sizeof(AnyOBJOnLine));
		...
		for (Y_two = 0; Y_two < SNES_HEIGHT_EXTENDED; Y_two++)
		{
			...
			GFX.OBJLines[Y_two].Tiles = S9xAuroraObjTileBudget();
...
{
	uint8_t	BGActive = Memory.FillRAM[0x212d] & S9xAuroraLayerMask();
	...
	if (BGActive & 0x10)
	{
		...
		BG.EnableMath = AURORA_COLOR_MATH(0);
		BG.StartPalette = 128;
	...
	BGActive = Memory.FillRAM[0x212c] & S9xAuroraLayerMask();
	D = 32;
	...
	if (BGActive & 0x10)
	{
		BG.TileAddress = PPU.OBJNameBase;
		BG.NameSelect = PPU.OBJNameSelect;
		BG.EnableMath = AURORA_COLOR_MATH((Memory.FillRAM[0x2131] & 0x10));
		BG.StartPalette = 128;
		S9xSelectTileConverter_Depth4();
		S9xSelectTileRenderers_SFXSpeedup();
		DrawOBJS(D + 4);
	...
	#define DO_BG_DEPTH2(n, pal, depth, hires, offset, Zh, Zl, voffoff) \
		...
		{ \
			BG.StartPalette = pal; \
			BG.EnableMath = AURORA_COLOR_MATH((Memory.FillRAM[0x2131] & (1 << n))); \
	...
	BG.EnableMath = AURORA_COLOR_MATH((Memory.FillRAM[0x2131] & 0x20));
}

static INLINE void RenderScreen (uint8_t sub)
{
	uint8_t	BGActive = Memory.FillRAM[0x212c+sub] & S9xAuroraLayerMask();
	int		D;
	...
	if (BGActive & 0x10)
	{
		BG.TileAddress = PPU.OBJNameBase;
		BG.NameSelect = PPU.OBJNameSelect;
		BG.EnableMath = AURORA_COLOR_MATH(!sub && (Memory.FillRAM[0x2131] & 0x10));
		BG.StartPalette = 128;
	...
	#define DO_BG_HIRES0_OFFSET0_D2(n, pal, depth, hires, offset, Zh, Zl, voffoff) \
		...
		{ \
			BG.StartPalette = pal; \
			BG.EnableMath = AURORA_COLOR_MATH(!sub && (Memory.FillRAM[0x2131] & (1 << n))); \
			BG.TileSizeH = (PPU.BG[n].BGSize) ? 16 : 8; \
			BG.TileSizeV = (PPU.BG[n].BGSize) ? 16 : 8; \
			S9xSelectTileConverter_Depth2(); \
			\
	...
	switch (PPU.BGMode)
	{
		...
		case 7:
			if (BGActive & 0x01)
			{
				BG.EnableMath = AURORA_COLOR_MATH(!sub && (Memory.FillRAM[0x2131] & 1));
				DrawBackgroundMode7(0, GFX.DrawMode7BG1Math, GFX.DrawMode7BG1Nomath, D);
			}

			if ((Memory.FillRAM[0x2133] & 0x40) && (BGActive & 0x02))
			{
				BG.EnableMath = AURORA_COLOR_MATH(!sub && (Memory.FillRAM[0x2131] & 2));
	...
	BG.EnableMath = AURORA_COLOR_MATH(!sub && (Memory.FillRAM[0x2131] & 0x20));
}
...
{
    /* Aurora's Window Effects=Off means an unclipped full-width region. */
    if (S9xAuroraHackFlags() & AURORA_HACK_WINDOWS_OFF)
    {
        int screen, layer;
        for (screen = 0; screen < 2; ++screen)
            for (layer = 0; layer < 6; ++layer)
            {
                IPPU.Clip[screen][layer].Count = 1;
                IPPU.Clip[screen][layer].Left[0] = 0;
                IPPU.Clip[screen][layer].Right[0] = 256;
                IPPU.Clip[screen][layer].DrawMode[0] = 3;
            }
        return;
    }
