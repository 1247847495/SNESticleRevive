...
| Device              | What it is                                                                                                                                                                                                          |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mass0:` / `mass1:` | **USB** (the PS2's two ports), USB **external HDD/SSD**, **MX4SIO** SD cards, and the **internal HDD in FAT/exFAT mode** (see below) — all block devices share the `massN:` namespace, numbered in detection order. |
...
### Internal HDD exFAT mode (内置硬盘 FAT/exFAT)

The internal PS2 Fat expansion-bay HDD can be used in **two mutually exclusive
modes**, switched from **Video Config → Storage / Devices**:

| Video Config switch   | Mode                                                                                 | What you get                                                                                                          |
| --------------------- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------- |
| **硬盘(APA)** (`hdd0:`) | APA/PFS partitions like HDD‑OSD / OPL                                                | The classic `hdd0:/PARTITION/` layout.                                                                                |
| **内置硬盘(FAT/exFAT)**   | The whole disk formatted as **FAT32 / exFAT** (MBR/GPT), listed as a `massN:` device | No APA partitioning needed — format the drive on a PC, copy ROMs over and it appears in the browser like a USB drive. |

Both modes own the ATA bus, so **enabling one automatically turns the other
off** — they cannot be used at the same time. The toggle is applied
immediately and persisted with the other Video Config settings. FAT32 and
exFAT are both supported (drives prepared with either filesystem work; MBR and
GPT partition tables are accepted).
...
translated to Simplified Chinese. Chinese ROM filenames are displayed
...
* **NES (InfoNES) integration**: full PS2 platform layer (render, input, audio,
  ...
  cycle-timed **Nes\_Snd\_Emu + Blip\_Buffer** at 32 kHz, and video uses Mesen2's
...
* **[game-music-emu](https://github.com/libgme/game-music-emu)** — Shay Green's Nes\_Snd\_Emu and Blip\_Buffer used for the five cycle-timed base NES audio channels (LGPL-2.1+).
...
* **anyi** — Simplified Chinese localization (中文汉化): menus, CJK font atlas, UTF-8 ROM filenames.
